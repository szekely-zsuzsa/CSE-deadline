...
# make timeout test responses FALSE in variable "correct", so that timeout responses count as incorrect responses
...
# create new variable storing whether the previous trials were correct
...
# calculate accuracy of respondents and create variable for accuracy-based exclusion
...
map(raw$response,
    . %>% 
      distinct(participant_id, wave, dropParticipantAcc) %>% 
      group_by(wave, dropParticipantAcc) %>% 
      summarise(n = n()))
```
...
map(raw$response,
    . %>% 
      ...
      distinct(participant_id, wave)) # 459 419
```
...
# Save practice, calibration and test trials to a separate dfs
...
# the number of trials after excluding the practice trials.
